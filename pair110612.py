...
def searchNearShape(wzpoint, data, already_paired, wztxt="info",mode="main",scale=50):
    ...
    result = []
    if len(choiceList) >= 1:
        choiceList.sort(key=lambda x:(x[1][0]-wzpoint[0])**2+(x[1][1]-wzpoint[1])**2, reverse=False)
        i = 0
            ...
            if shape["label"] in ["wz1","wz3"]:
                ...
                while nearest_shape is None:
                    if line_endPoint is not None:
                        nearest_shape = searchNearShape(line_endPoint,data,already_paired,txt,mode="test",scale=search_scale)
                        if search_scale > 200:
                            res_label="NotFound"
                            res_point=line_endPoint
                            break
                    else:
                        nearest_shape = searchNearShape(ct_wz,data,already_paired,txt,mode="test",scale=search_scale)
                        if search_scale > 500:
                            res_label="NotFound"
                            res_point=line_endPoint
                            break
                ...
                ## draw_out_rectangle写入临时json，用于观察搜索范围
                # draw_out_rectangle = {"label": "range","shape_type": "rectangle",
                #                     "points": [[ct_wz[0] - search_scale,ct_wz[1] - search_scale],
                #                                [ct_wz[0] + search_scale,ct_wz[1] + search_scale]],"flags": {},"group_id": None}
                # data["shapes"].append(draw_out_rectangle)
                ...
                print('--------------'*2+"||"+"--------------"*2)
                print('--------------'*2+"\/"+"--------------"*2)
                print("Piclabel: {:<9s} coordinate: {} ".format(nearest_shape[0]['label'],nearest_shape[0]['points']))
                print(" ")
...
if __name__ == "__main__":
    ...
    singleMatch(path,"00-三角塔-汇总_0.json",search_scale=100, ocr_flag=False)
    # mainProc(path,files,ocr_flag=False)                    
    showResult(path,["00-三角塔-汇总_0.json"],dst="C:\\Users\\19586\\Downloads\\pairWZ\\result")
